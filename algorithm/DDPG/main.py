...
if __name__ == "__main__":
    ...
    # parser.add_argument("--env_name", default="Ant-v4")  # OpenAI gym environment name
    # parser.add_argument("--env_name", default="Hopper-v3")  # OpenAI gym environment name
    parser.add_argument("--env_name", default="InvertedDoublePendulum-v4")  # OpenAI gym environment name
    ...
    log_dir = args.policy + "_logs/" + args.env_name + '/'
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    t_now = time.time()
    time_str = datetime.fromtimestamp(t_now).strftime('%Y%m%d%H%M%S')
    log_f_name = log_dir + args.policy + '_' + args.env_name + "_log_" + time_str + ".csv"
